...
Для синхронизации выполняется следующая команда:  pass git pull pass git push Следует заметить, что отслеживаются только изменения, сделанные через сам gopass (или pass). Если изменения сделаны непосредственно на файловой системе, необходимо вручную закоммитить и выложить изменения. Проверим статус синхронизации. (рис. @fig:005).

![Настройка синхронизации с git](image/4.PNG){#fig:005 width=70%}
...
![установка bugzilla browserpass] (image/5.PNG){#fig:006 width=70%}

задаю пароль потом вывожу пароль с помощью команды pass <name> и потом генерирую новый (рис. @fig:007).

![пароли](image/6.PNG){#fig:007 width=70%}

устанавливаю все расширения (рис. @fig:008).

![установка](image/7.PNG){#fig:008 width=70%}

chezmoi (рис. @fig:009).

![использование программы](image/8.PNG){#fig:009 width=70%} 

выполняю команды chezmoi, которые присутсвуют в лабораторной работе, но все данные уже актуальны, благодаря предыдущим действиям (рис. @fig:010).

![chezmoi](image/9.PNG){#fig:010 width=70%} 
